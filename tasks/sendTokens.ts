...
task('lz:oft:send', 'Send tokens cross-chain using LayerZero technology').setAction(async (taskArgs, hre) => {
    ...
    /** */
    const oftContractFactoryA = await hre.ethers.getContractFactory('DebtTokenWithLz', walletA)
    const oftA = oftContractFactoryA.attach(contractA)
    const oftContractFactoryB = await hre.ethers.getContractFactory('DebtTokenWithLz', walletB)
