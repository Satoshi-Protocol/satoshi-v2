...
contract TroveManager is ITroveManager, Initializable, OwnableUpgradeable {
        ...
        public
    {
        ...
        require(_borrowingFeeFloor <= _maxBorrowingFee && _maxBorrowingFee <= SatoshiMath.DECIMAL_PRECISION);
    ...
    function getPendingCollAndDebtRewards(address _borrower) public view returns (uint256, uint256) {
        ...
        return ((stake * coll) / SatoshiMath.DECIMAL_PRECISION, (stake * debt) / SatoshiMath.DECIMAL_PRECISION);
    }
        ...
        returns (uint256)
    {
        ...
        newBaseRate = SatoshiMath._min(newBaseRate, SatoshiMath.DECIMAL_PRECISION); // cap baseRate at a maximum of 100%
    ...
    function _calcBorrowingFee(uint256 _borrowingRate, uint256 _debt) internal pure returns (uint256) {
        return (_borrowingRate * _debt) / SatoshiMath.DECIMAL_PRECISION;
    }
    ...
    function _calcDecayedBaseRate() internal view returns (uint256) {
        ...
        return (baseRate * decayFactor) / SatoshiMath.DECIMAL_PRECISION;
    }
    ...
    function _redistributeDebtAndColl(uint256 _debt, uint256 _coll) internal {
         ...
         */
        uint256 collateralNumerator = (_coll * SatoshiMath.DECIMAL_PRECISION) + lastCollateralError_Redistribution;
        uint256 debtNumerator = (_debt * SatoshiMath.DECIMAL_PRECISION) + lastDebtError_Redistribution;
