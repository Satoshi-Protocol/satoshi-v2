...
import {IUniswapV2Router01} from "../../library/interfaces/vault/IUniswapV2Router01.sol";
...
contract UniV2Vault is VaultCore {
    ...
    function initialize(bytes calldata data) external override initializer {
        ...
        (address stableTokenAddress_, address satAddress_, address pair_) =
    ...
    function executeStrategy(bytes calldata data) external override onlyOwner {
        ...
        INexusYieldManagerFacet(nymAddr).swapInPrivileged(STABLE_TOKEN_ADDRESS, address(this), amountA);
    ...
    function exitStrategy(bytes calldata data) external override onlyOwner returns (uint256) {
        ...
        uint256 previewAmount = INexusYieldManagerFacet(nymAddr).convertDebtTokenToAssetAmount(
            STABLE_TOKEN_ADDRESS, IERC20(SAT_ADDRESS).balanceOf(address(this))
        );
        uint256 swapOutAmount =
            INexusYieldManagerFacet(nymAddr).swapOutPrivileged(STABLE_TOKEN_ADDRESS, address(this), previewAmount);
